...
description: 使用 ModelOriented/EloML 原生强度流程和独立的俱乐部主客场攻防比分层，分析英超、西甲及其他欧洲五大联赛。用于赛前预测、临场复查、赛后复盘，或需要综合严格 EloML、90 分钟胜平负、让球、大小球、阵容、赛程密度、欧战影响和三个比分时。英超与西甲优先，联赛数据和世界杯模型必须彼此独立。
---
...
- 只分析联赛 90 分钟结果。足总杯、国王杯、欧冠等杯赛另行标注，不直接套用联赛口径。
- 国家队和世界杯继续使用 `worldcup-eloml-predictor`；不得混用数据、脚本或模板。

## 工作流

1. 确认联赛、主客队、开球时间和用户时区。“今天、现在、最新、临场、走地”必须联网核实。
2. 获取最新完赛赛果，并排除目标比赛日期当天及之后的数据。优先使用最近三个完整/进行中赛季，同一联赛单独建模。
3. 运行俱乐部模型。例如：
